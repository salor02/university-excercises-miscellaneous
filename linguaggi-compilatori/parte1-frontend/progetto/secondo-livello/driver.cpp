...
/*TODO:
1)  Initexp empty fa crashare (esempio var i;) */
...
Value* ForStmtAST::codegen(driver& drv) {
    ...
    // viene restituito il valore finale dell'ultimo assegnamento effettuato
    return var;
}
...
AssignmentAST::AssignmentAST(const std::string Name, ExprAST* Val):
   Name(Name), Val(Val) { definition = false; };
...
VarBindingAST::VarBindingAST(const std::string Name, ExprAST* Val):
    Name(Name), Val(Val) { definition = true; };
